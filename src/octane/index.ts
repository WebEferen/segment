// @octanejs/segment — the Octane binding.
//
...
function ownerOf(ref: { readonly path: string }): Store<unknown> {
	...
	if (store === null) {
		throw new Error(
			'[@octanejs/segment] this ref has no owning store. Build refs from ' +
...
const NOT_WRITABLE = () => {
	throw new Error(
		'[@octanejs/segment] this value cannot be written. You can write anything you ' +
...
function bindingFor(ref: AnyRef, store: Store<unknown>, slot: symbol | undefined): Binding {
	return useMemo(
		() => {
			...
			if (kind === 'action') {
				throw new Error(
					`[@octanejs/segment] "${ref.path}" is something you call, not something you ` +
						'read. Call it directly: `s.cart.add("sku")`.',
				);
			}
			if (kind === 'resource') {
				throw new Error(
					`[@octanejs/segment] "${ref.path}" is fetched, so reading it means waiting. ` +
...
// worker or a server route can pull the same names from `@octanejs/segment/core`
